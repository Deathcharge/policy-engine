...
class ExecutionLoop:
    ...
    async def execute(self, plan: Plan, executor: Any, ucf_context: UCFMetrics) -> ExecutionResult:
        ...
        return ExecutionResult(
            ...
            metadata={
                "metrics": metrics,
                "ucf_metrics": ucf_context.to_dict(),
            },
    ...
    async def reflect(self, result: ExecutionResult, original_goal: str, ucf_context: UCFMetrics) -> str:
        ...
        execution_metrics = result.metrics or result.metadata.get("metrics", {})

        if result.success:
            reflection = f"Successfully achieved goal: {original_goal}. "
            reflection += f"Completed {execution_metrics.get('steps_completed', 0)} steps. "
    ...
    async def evaluate_outcome(self, outcome: ExecutionResult, expected_result: Any) -> float:
        ...
        base_score = 0.5
        outcome_ucf = outcome.ucf_metrics or UCFMetrics()
        base_score += outcome_ucf.harmony * 0.3
        base_score += outcome_ucf.resilience * 0.2
    ...
    async def run_cognitive_loop(
        ...
        ucf_trajectory: list[UCFMetrics] = []
        reflections: list[str] = []
        result = ExecutionResult(success=False, result=None, execution_time=0.0, error="Loop not executed")
        ...
        for iteration in range(max_iterations):
            ...
            # Track UCF
            ucf_trajectory.append(
                UCFMetrics(
                    velocity=ucf_context.velocity,
                    harmony=ucf_context.harmony,
                    resilience=ucf_context.resilience,
                    throughput=ucf_context.throughput,
                    focus=ucf_context.focus,
                    friction=ucf_context.friction,
                )
            )
        ...
        loop_result.final_result = result.output
        loop_result.iterations = completed_iterations
        loop_result.reflections = reflections
        loop_result.total_time = total_time
        loop_result.ucf_trajectory = ucf_trajectory
        return loop_result
