...
_CREDENTIAL_PATTERN = re.compile(
    r"(sk-[a-zA-Z0-9]{20,}|Bearer\s+\S{20,}|password\s*[:=]\s*\S+|api[_-]?key\s*[:=]\s*\S{10,})",
    re.IGNORECASE,
)
...
class TaskOrchestrator:
    ...
    def _validate_pinned_constraints(self, task: Task) -> None:
        ...
        if (task_context.get("sensitive") or task_context.get("destructive")) and not task_context.get(
