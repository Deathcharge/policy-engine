...
if TYPE_CHECKING:
    from ..proprietary_llm import HelixInferenceEngine

logger = logging.getLogger(__name__)


def _load_proprietary_llm_runtime() -> tuple[bool, Any | None]:
    try:
        from ..proprietary_llm import TORCH_AVAILABLE, initialize_helix_llm_engine

        return TORCH_AVAILABLE, initialize_helix_llm_engine
    except (ImportError, ValueError):
        return False, None
...
class HelixCoreLLMBridge:
    ...
    def __init__(self):
        ...
        self._model_mapping = {
            ...
            Tier.BUILDER: ["gpt-3.5-turbo", "claude-instant", "gpt-4", "grok-3-mini"],
    ...
    async def initialize(self) -> bool:
        ...
        try:
            torch_available, initialize_helix_llm_engine = _load_proprietary_llm_runtime()
            if torch_available and initialize_helix_llm_engine:
    ...
    async def generate(
        ...
        torch_available, _ = _load_proprietary_llm_runtime()
    ...
    async def _inference_external(self, prompt: str, model: str, max_tokens: int, temperature: float) -> tuple:
            ...
            try:
                ...
                if api_key:
                    ...
                    tokens = (
                        ...
                        if response.usage
                        else math.ceil(len(prompt) / 4) + math.ceil(len(content) / 4)
                    )
                    return content, tokens
            except ImportError:
        ...
        tokens = math.ceil(len(prompt) / 4) + math.ceil(len(content) / 4)
        return content, tokens
