...
class UCFAdapter:
    ...
    def __init__(self, ucf_store: UCFStore):
        ...
        self._event_handlers: dict[str, list[Callable[[str, str, UCFMetrics], Awaitable[Any]]]] = {}
        self._adapter_lock = asyncio.Lock()

        logger.info("UCFAdapter initialized")

    async def _sync_from_metric_payload(
        self,
        agent_id: str,
        ...
        *,
        ...
        """Store metric payloads and emit the requested synchronization event."""
        metrics = UCFMetrics()

        if "velocity" in metric_payload:
            metrics.velocity = float(metric_payload["velocity"])

        if "harmony" in metric_payload:
            metrics.harmony = float(metric_payload["harmony"])

        if "resilience" in metric_payload:
            metrics.resilience = float(metric_payload["resilience"])

        if "throughput" in metric_payload:
            metrics.throughput = float(metric_payload["throughput"])

        if "focus" in metric_payload:
            metrics.focus = float(metric_payload["focus"])

        if "friction" in metric_payload:
            metrics.friction = float(metric_payload["friction"])

        await self._ucf_store.set_metrics(agent_id, metrics)
        await self._emit_event(event_name, agent_id, metrics)

        logger.info("Synced UCF metrics from %s for agent %s", source_label, agent_id)
        return metrics

    async def sync_from_external_metrics(self, agent_id: str, external_metrics: dict[str, Any]) -> UCFMetrics:
        """Sync UCF metrics from an external payload using canonical field names."""
        return await self._sync_from_metric_payload(
            agent_id,
            external_metrics,
            event_name="sync_from_external_metrics",
            source_label="external metrics",
        )
    ...
    async def sync_from_legacy(self, agent_id: str, legacy_data: dict[str, Any]) -> UCFMetrics:
        ...
        return await self._sync_from_metric_payload(
            agent_id,
            legacy_data,
            event_name="sync_from_legacy",
            source_label="legacy format",
        )

    async def _sync_to_metric_payload(
        self,
        agent_id: str,
        *,
        event_name: str,
        ...
        """Load canonical metrics, then emit the requested synchronization event."""
        ...
        logger.info("Synced UCF metrics to %s for agent %s", target_label, agent_id)
        return metric_payload

    async def sync_to_external_metrics(self, agent_id: str) -> dict[str, Any] | None:
        """Export UCF metrics using the canonical external payload shape."""
        return await self._sync_to_metric_payload(
            agent_id,
            event_name="sync_to_external_metrics",
            target_label="external metrics",
        )
    ...
    async def sync_to_legacy(self, agent_id: str) -> dict[str, Any] | None:
        ...
        return await self._sync_to_metric_payload(
            agent_id,
            event_name="sync_to_legacy",
            target_label="legacy format",
        )
    ...
    async def subscribe_to_events(
        self,
        event_type: str,
        handler: Callable[[str, str, UCFMetrics], Awaitable[Any]],
    ) -> None:
        """
    ...
    async def _emit_event(self, event_type: str, agent_id: str, metrics: UCFMetrics) -> None:
